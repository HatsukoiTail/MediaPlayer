...
enum class MediaInfo::DecodeResult
{
    FrameReady,
    NeedMoreData,
    EndOfStream,
...
MediaInfo::MediaInfo(const QString &path)
{
    this->open(path);
}
...
bool MediaInfo::open(const QString &path)
{
    ...
    const auto meta_data = this->data_stream.metaData();
    this->type = formatMetaType(meta_data["metaType"].toString().toStdString());
    if (this->type != MetaType::Video && this->type != MetaType::Audio)
    {
        this->is_opened = true;
        return true;
    }
...
bool MediaInfo::isOpen() const
{
    return this->is_opened;
}

QImage MediaInfo::thumbnail()
{
    assert(this->is_opened && this->format_ctx && this->codec_ctx);
    assert(this->type == MetaType::Video);
    ...
    while (looping)
    {
        ...
        case DecodeResult::EndOfStream:
            return {};
        default:
            print(Ansi::Red, "Error occurred when decode");
            return {};
        }
    }
    if (!frame)
...
QImage MediaInfo::image()
{
    assert(this->is_opened && this->type == MetaType::Image);
    auto data = this->data_stream.readAll();
...
MediaData MediaInfo::mediaData(const QString &path)
{
    MediaInfo media(path);
    if (!media.isOpen())
        return {};
    return media.mediaData();
}

AVPacketPointer MediaInfo::read_packet()
{
...
std::pair<MediaInfo::DecodeResult, AVFramePointer> MediaInfo::read_frame(AVPacketPointer packet)
{
...
AVFramePointer MediaInfo::sws_frame(AVFramePointer frame)
{
...
int MediaInfo::read_callback(void *opaque, uint8_t *buf, int buf_size)
{
    auto media_data = reinterpret_cast<MediaInfo*>(opaque);
...
int64_t MediaInfo::seek_callback(void *opaque, int64_t offset, int whence)
{
    auto media_data = reinterpret_cast<MediaInfo*>(opaque);
